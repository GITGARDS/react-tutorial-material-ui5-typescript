...
const getAll = async (
  ...
  try {
    const urlRelativa = `${urlService}?_page=${page}&_limit=${Environment.LIMITE_DE_LINHAS}&nome_like=${filter}`;
...
const create = async (
  ...
  try {
    const { data } = await Api.post<IDetalheCidade>(`${urlService}`, dados);
